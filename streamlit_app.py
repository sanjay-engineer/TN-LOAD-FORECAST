# ================================================================
#  TN LOAD FORECASTING — STREAMLIT APP WITH ROLLING FORECAST
#
#  Rolling forecast features:
#  • Auto-reads rolling_status.json to know the active window
#  • Shows which months are actual vs forecast
#  • Admin can upload actual CSV files to trigger re-forecast
#  • Accuracy tab shows MAPE/RMSE for months with actual data
#  • Dynamic forecast window: always next 3 months after actuals
#
#  TABS:
#  1. 🏠 Overview      — rolling status + summary metrics
#  2. 📈 Daily         — today vs actual + tomorrow hourly
#  3. 📅 Monthly       — per-month bar+line + day picker
#  4. 📊 5-Year        — comparison vs 2020-2025
#  5. 🎯 Accuracy      — MAPE/RMSE for months with actual data
#  6. 📋 All Results   — full table + download
# ================================================================
...
MONTH_COLORS = {
    1: '#0ea5e9', 2: '#6366f1', 3: '#8b5cf6',
    4: '#2563eb', 5: '#16a34a', 6: '#ea580c',
    7: '#f59e0b', 8: '#ec4899', 9: '#06b6d4',
    10: '#64748b', 11: '#7c3aed', 12: '#dc2626',
}
MONTH_FILL = {
    4: 'rgba(37,99,235,0.10)', 5: 'rgba(22,163,74,0.10)',
    6: 'rgba(234,88,12,0.10)', 7: 'rgba(245,158,11,0.10)',
    8: 'rgba(236,72,153,0.10)',
}
YEAR_COLORS = {
...
def read_json(fn):
    ...
    loc = os.path.join(SHARED_DIR, fn)
    if os.path.exists(loc):
        with open(loc) as f: return json.load(f)
    return None

def load_rolling(): return read_csv("rolling_results.csv")
def load_status():  return read_json("rolling_status.json")
def load_mo(mn, yr): return read_csv(f"{mn.lower()}_{yr}_results.csv")
...
def show_login():
    st.markdown(
        ...
        "<p style='color:#64748b;font-size:13px'>Tamil Nadu Power Grid — Rolling LSTM Forecast System</p>"
...
# ── SIDEBAR ───────────────────────────────────────────────────
def show_sidebar(un, role, status):
    with st.sidebar:
        ...
        # Rolling forecast window status
        st.divider()
        st.subheader("🔄 Rolling Window")
        if status:
            base_yr = status.get('base_end_year', '?')
            base_mo = status.get('base_end_month', '?')
            st.markdown(f"**History ends:** {MONTH_NAMES.get(base_mo,'?')} {base_yr}")
            confirmed = status.get('confirmed_actuals', [])
            if confirmed:
                st.markdown("**✅ Actual data:**")
                for a in confirmed:
                    st.markdown(f"  • {MONTH_NAMES.get(a['month'],'?')} {a['year']}")
            fc_months = status.get('forecast_months', [])
            if fc_months:
                st.markdown("**🔮 Forecasting:**")
                for f in fc_months:
                    c = MONTH_COLORS.get(f['month'], '#2563eb')
                    st.markdown(
                        f"<span style='color:{c}'>● {MONTH_NAMES.get(f['month'],'?')} "
                        f"{f['year']}</span>", unsafe_allow_html=True)
        else:
            st.info("Run Colab to generate rolling status")
        ...
        if role == "admin":
            st.divider()
            with st.expander("📂 Upload Forecast Results"):
                st.caption("Upload CSV files generated by the Colab notebook")
                for fn, k in [
                    ("rolling_results.csv",    "ru"),
                    ("rolling_status.json",    "rs"),
                ]:
                    uf = st.file_uploader(fn, type=["csv", "json"], key=k)
                    if uf:
                        n = save_local(uf, fn); st.success(f"✓ {fn} ({n} rows)")
            with st.expander("📂 Upload Month Results"):
                st.caption("Upload per-month result CSVs")
                month_files = [
                    ("april_2026_results.csv", "a26"),
                    ("may_2026_results.csv",   "m26"),
                    ("june_2026_results.csv",  "j26"),
                    ("july_2026_results.csv",  "j226"),
                    ("august_2026_results.csv","ag26"),
                ]
                for fn, k in month_files:
...
def show_dashboard(un, role):
    ...
    # Page header
    st.markdown(
        f"<h2 style='color:#2563eb'>⚡ TN Load Forecasting — Rolling Dashboard</h2>"
        f"<p style='color:#64748b'>Tamil Nadu Power Grid · Welcome <b>{un}</b></p>",
        unsafe_allow_html=True)

    # Rolling forecast status banner
    if status:
        fc = status.get('forecast_months', [])
        ca = status.get('confirmed_actuals', [])
        be_yr = status.get('base_end_year', '?')
        be_mo = status.get('base_end_month', '?')
        fc_str = " → ".join(
            f"{MONTH_NAMES.get(f['month'], '?')} {f['year']}" for f in fc)
        ca_str = (", ".join(
            f"{MONTH_NAMES.get(a['month'], '?')} {a['year']}" for a in ca)
            if ca else "None yet")
        st.info(
            f"**🔄 Rolling Window Active** — "
            f"History through: **{MONTH_NAMES.get(be_mo,'?')} {be_yr}** | "
            f"Actuals confirmed: **{ca_str}** | "
            f"Forecast window: **{fc_str}**"
        )
    else:
        st.warning("⚠ No rolling status found. Run the Colab notebook to generate forecasts.")

    st.divider()

    # Determine forecast months from status or default
    if status and status.get('forecast_months'):
        fc_months_list = [(f['year'], f['month'])
                         for f in status['forecast_months']]
    else:
        fc_months_list = [(2026, 4), (2026, 5), (2026, 6)]

    # Parse rolling results
    if df_roll is not None and len(df_roll) > 0:
        df_roll['has_actual'] = df_roll.get('has_actual', pd.Series([False]*len(df_roll)))
        df_past   = df_roll[df_roll.get('has_actual', False) == True].copy() if 'has_actual' in df_roll.columns else pd.DataFrame()
        df_future = df_roll.copy()
        df_m      = df_roll[df_roll['mape'].notna()].copy() if 'mape' in df_roll.columns else pd.DataFrame()
    else:
        df_past = df_future = df_m = pd.DataFrame()

    # KPI row
    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("📅 History", "2020–2025")
    c2.metric("📊 Rows", "54,768")
    c3.metric("🔄 Mode", "Rolling")
    if status:
        n_actuals = len(status.get('confirmed_actuals', []))
        c4.metric("✅ Actual Months", n_actuals)
        n_fc = len(status.get('forecast_months', []))
        c5.metric("🔮 Forecast Months", n_fc)
    else:
        c4.metric("✅ Forecast Days", len(df_roll) if df_roll is not None else "—")
        c5.metric("🔮 Target", "3 Months")
    if len(df_m) > 0:
        c6.metric("🎯 Avg MAPE", f"{df_m['mape'].mean():.2f}%")
    else:
        c6.metric("🎯 MAPE", "—")
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
        ...
        "📅 Monthly Forecast", "📊 5-Year Comparison",
        "🎯 Accuracy", "📋 All Results"
    ])

    # ── TAB 1: OVERVIEW ──────────────────────────────────────────
    with tab1:
        st.subheader("🔄 Rolling Forecast Overview")

        if status:
            fc_months = status.get('forecast_months', [])
            ca_months = status.get('confirmed_actuals', [])

            # Show each forecast month card
            cols = st.columns(len(fc_months) if fc_months else 1)
            for i, fm in enumerate(fc_months):
                yr, mo = fm['year'], fm['month']
                mn     = MONTH_NAMES.get(mo, '?')
                color  = MONTH_COLORS.get(mo, '#2563eb')
                df_mo  = (load_mo(mn, yr) if df_roll is not None else None)
                with cols[i]:
                    if df_mo is not None and len(df_mo) > 0:
                        ...
                        mape = (pd.to_numeric(df_mo['mape'], errors='coerce').mean()
                                if 'mape' in df_mo.columns else None)
                        ha   = (df_mo['has_actual'].any()
                                if 'has_actual' in df_mo.columns else False)
                        badge = "✅ Actual + Forecast" if ha else "🔮 Forecast Only"
                        mape_str = (f"MAPE: {mape:.2f}%" if mape and not np.isnan(mape)
                                    else "MAPE: —")
                        st.markdown(
                            ...
                            f"{mn} {yr}</div>"
                            f"<div style='font-size:12px;color:#64748b;margin:4px 0'>{badge}</div>"
                            f"<div style='font-size:28px;font-weight:bold;color:#1e293b'>"
                            f"{avg:,.0f} MW</div>"
                            f"<div style='font-size:13px;color:#64748b'>Avg Load</div>"
                            f"<div style='font-size:20px;color:#dc2626;margin-top:6px'>"
                            f"{peak:,.0f} MW</div>"
                            f"<div style='font-size:12px;color:#64748b'>Peak Load</div>"
                            f"<div style='font-size:12px;color:{color};margin-top:8px'>"
                            f"{mape_str}</div>"
                            f"</div>", unsafe_allow_html=True)
                    else:
                        st.markdown(
                            f"<div style='background:#f8fafc;border:2px dashed {color};"
                            f"border-radius:12px;padding:18px;text-align:center'>"
                            f"<div style='font-size:22px;font-weight:bold;color:{color}'>"
                            f"{mn} {yr}</div>"
                            f"<div style='font-size:14px;color:#94a3b8;margin:20px 0'>"
                            f"Run Colab notebook to generate forecast</div>"
                            f"</div>", unsafe_allow_html=True)

            st.divider()

            # Confirmed actuals section
            if ca_months:
                st.subheader("✅ Confirmed Actual Months")
                st.caption(
                    "These months have real data — used to improve future forecasts")
                ca_cols = st.columns(len(ca_months))
                for i, ca in enumerate(ca_months):
                    yr, mo = ca['year'], ca['month']
                    mn     = MONTH_NAMES.get(mo, '?')
                    color  = MONTH_COLORS.get(mo, '#2563eb')
                    df_mo  = load_mo(mn, yr)
                    with ca_cols[i]:
                        if df_mo is not None and 'mape' in df_mo.columns:
                            mape = pd.to_numeric(df_mo['mape'], errors='coerce').mean()
                            rmse = pd.to_numeric(df_mo['rmse'], errors='coerce').mean()
                            st.markdown(
                                f"<div style='background:#f0fdf4;border:2px solid #16a34a;"
                                f"border-radius:12px;padding:16px;text-align:center'>"
                                f"<div style='font-size:18px;font-weight:bold;color:#16a34a'>"
                                f"✓ {mn} {yr}</div>"
                                f"<div style='font-size:22px;font-weight:bold;color:#1e293b'>"
                                f"MAPE: {mape:.2f}%</div>"
                                f"<div style='font-size:14px;color:#64748b'>"
                                f"RMSE: {rmse:.0f} MW</div>"
                                f"</div>", unsafe_allow_html=True)
                        else:
                            st.markdown(
                                f"<div style='background:#f0fdf4;border:2px solid #16a34a;"
                                ...
                                f"<div style='font-size:18px;font-weight:bold;color:#16a34a'>"
                                f"✓ {mn} {yr}</div>"
                                f"<div style='color:#64748b'>Accuracy data not loaded</div>"
                                f"</div>", unsafe_allow_html=True)
        else:
            st.info(
                "**How rolling forecasting works:**\n\n"
                "1️⃣ **Run 1 (no actuals):** Train on Jan 2020 – Dec 2025 → "
                "Forecast **Apr, May, Jun 2026**\n\n"
                "2️⃣ **Run 2 (after April ends):** Upload `actual_april_2026.csv` → "
                "Compute accuracy → Retrain → Forecast **May, Jun, Jul 2026**\n\n"
                "3️⃣ **Run 3 (after May ends):** Upload `actual_may_2026.csv` → "
                "Compute accuracy → Retrain → Forecast **Jun, Jul, Aug 2026**\n\n"
                "The window always rolls forward — always 3 months ahead of your latest real data."
            )
    ...
    with tab2:
        ...
        if df_roll is None or len(df_roll) == 0:
            st.info("No forecast data. Run Colab first.")
        else:
            # Most recent day with actual data
            if 'has_actual' in df_roll.columns:
                df_with_act = df_roll[df_roll['has_actual'] == True]
            else:
                df_with_act = pd.DataFrame()

            if len(df_with_act) > 0:
                row = df_with_act.iloc[-1]
                pred   = g24(row, 'pred')
                actual = g24(row, 'actual')
                mv = sf(row.get('mape')); rv = sf(row.get('rmse'))
                vp = [v for v in pred if v]
                m1, m2, m3, m4 = st.columns(4)
                if mv:
                    col = "green" if mv < 5 else "orange" if mv < 10 else "red"
                    m1.markdown(f"<h3 style='color:{col}'>{mv:.2f}%</h3>"
                                f"<p style='color:#64748b;font-size:12px'>MAPE</p>",
                                unsafe_allow_html=True)
                if rv:
                    m2.markdown(f"<h3>{rv:.0f} MW</h3>"
                ...
                m4.markdown(f"<h3>{row['date']}</h3>"
                            ...
                            unsafe_allow_html=True)
                ft = go.Figure()
                if any(v for v in actual):
                    ft.add_trace(go.Scatter(x=hlbl, y=actual, name="Actual",
                                            line=dict(color="#16a34a", width=3),
                                            mode="lines+markers", marker=dict(size=7),
                                            fill="tozeroy", fillcolor="rgba(22,163,74,0.07)"))
                ft.add_trace(go.Scatter(x=hlbl, y=pred, name="Predicted",
                                        line=dict(color="#2563eb", width=2.5, dash="dash"),
                                        mode="lines+markers", marker=dict(size=6)))
                ft.update_layout(title=f"Predicted vs Actual — {row['date']}",
                                 xaxis_title="Hour", yaxis_title="Load (MW)",
                                 height=380, **BL)
                st.plotly_chart(ft, use_container_width=True)
            else:
                st.info("No days with actual data yet. Forecasts only.")

            # Tomorrow (next unverified day)
            st.divider()
            st.subheader("🔮 Next Forecast Day")
            if 'has_actual' in df_roll.columns:
                df_fc = df_roll[df_roll['has_actual'] != True]
            else:
                df_fc = df_roll
            if len(df_fc) > 0:
                nr  = df_fc.iloc[0]
                np_ = g24(nr, 'pred')
                vnp = [v for v in np_ if v]
                ...
                if vnp:
                    ...
                    n3.markdown(f"<h3>{np.mean(vnp):,.0f} MW</h3>"
                                f"<p style='color:#64748b;font-size:12px'>Average</p>",
                                unsafe_allow_html=True)
                n4.markdown(f"<h3 style='color:#7c3aed'>{nr['date']}</h3>"
                            f"<p style='color:#64748b;font-size:12px'>Date</p>",
                            unsafe_allow_html=True)
                fn2 = go.Figure()
                fn2.add_trace(go.Scatter(x=hlbl, y=np_, name="Forecast",
                ...
                if vnp:
                    ...
                    fn2.add_annotation(x=hlbl[ph], y=max(vnp),
                                       text=f"Peak:{max(vnp):,.0f} MW",
                                       showarrow=True, arrowhead=2,
                                       arrowcolor="#ea580c",
                                       font=dict(color="#ea580c", size=11),
                                       bgcolor="white",
                                       bordercolor="#ea580c", borderwidth=1.5, ay=-40)
                fn2.update_layout(title=f"Next Forecast — {nr['date']}",
                                  xaxis_title="Hour", yaxis_title="Load (MW)",
                                  height=380, **BL)
    ...
    with tab3:
        ...
        for yr, mo in fc_months_list:
            ...
            if df_mo is not None and len(df_mo) > 0:
                ...
                mo_data[(yr, mo)] = (mn, df_mo)

        if not mo_data:
            st.info("Forecast data not yet available. Run the Colab notebook.")
        else:
            # Combined 3-month view
            st.markdown("### Combined Forecast View")
            cb, cl = st.columns(2)
            with cb:
                fc2 = go.Figure(); xoff = 0
                for (yr, mo), (mn, df_mo) in mo_data.items():
                    color = MONTH_COLORS.get(mo, '#2563eb')
                    xs    = [xoff + d for d in df_mo['day']]
                    avgs  = pd.to_numeric(df_mo['predicted_avg'], errors='coerce').tolist()
                    fc2.add_trace(go.Bar(x=xs, y=avgs, name=f"{mn} {yr}",
                                         marker_color=color, opacity=0.85, width=0.8))
                    if xoff > 0:
                        fc2.add_vline(x=xoff + 0.5, line_dash="dash",
                                      line_color="gray", opacity=0.4)
                    xoff += calendar.monthrange(yr, mo)[1]
                fc2.update_layout(title="Daily Avg Load — Bar", xaxis_title="Day",
                                  yaxis_title="Avg Load (MW)", height=360, **BL)
                st.plotly_chart(fc2, use_container_width=True)
            with cl:
                fl2 = go.Figure(); xoff = 0
                for (yr, mo), (mn, df_mo) in mo_data.items():
                    color = MONTH_COLORS.get(mo, '#2563eb')
                    fill  = MONTH_FILL.get(mo, 'rgba(37,99,235,0.10)')
                    xs    = [xoff + d for d in df_mo['day']]
                    avgs  = pd.to_numeric(df_mo['predicted_avg'], errors='coerce').tolist()
                    fl2.add_trace(go.Scatter(x=xs, y=avgs, name=f"{mn} {yr}",
                                             line=dict(color=color, width=2.5),
                                             mode="lines+markers", marker=dict(size=4),
                                             fill="tozeroy", fillcolor=fill))
                    if xoff > 0:
                        fl2.add_vline(x=xoff + 0.5, line_dash="dash",
                                      line_color="gray", opacity=0.4)
                    xoff += calendar.monthrange(yr, mo)[1]
                fl2.update_layout(title="Daily Avg Load — Line",
            ...
            for (yr, mo), (mn, df_mo) in mo_data.items():
                ...
                has_a = ('has_actual' in df_mo.columns and df_mo['has_actual'].any())
                act_a = (pd.to_numeric(df_mo.get('actual_avg', []), errors='coerce').tolist()
                         if has_a else [])
                prev_avgs = [HIST_DAILY.get((yr - 1, mo), {}).get(d) for d in days]

                badge = "✅ Actual + Forecast" if has_a else "🔮 Forecast Only"
                st.markdown(
                    f"<div style='background:{color};color:white;padding:10px 18px;"
                    f"border-radius:8px;margin:18px 0 10px 0'>"
                    f"<b>📅 {mn} {yr}</b> — {ndays} days &nbsp;|&nbsp; "
                ...
                if has_a and 'mape' in df_mo.columns:
                    mape_val = pd.to_numeric(df_mo['mape'], errors='coerce').mean()
                    m3.metric("Avg MAPE",
                              f"{mape_val:.2f}%" if not np.isnan(mape_val) else "—")
                else:
                ...
                with bc:
                    fb = go.Figure()
                    fb.add_trace(go.Bar(x=days, y=avgs, name=f"Forecast {yr}",
                                        marker_color=color, opacity=0.85,
                                        text=[f"{v:,.0f}" if v else "" for v in avgs],
                                        textposition='outside'))
                    if has_a and act_a:
                        fb.add_trace(go.Bar(x=days, y=act_a, name=f"Actual {yr}",
                                            marker_color='#16a34a', opacity=0.55))
                    if prev_avgs:
                        fb.add_trace(go.Bar(x=days, y=prev_avgs,
                                            name=f"{yr - 1} Actual",
                                            marker_color=YEAR_COLORS.get(yr - 1, '#94a3b8'),
                                            opacity=0.4))
                    fb.add_trace(go.Scatter(x=days, y=peaks, name="Forecast Peak",
                                            mode="lines+markers",
                                            line=dict(color="#dc2626", width=2, dash="dot"),
                                            marker=dict(size=5, color="#dc2626")))
                    fb.update_layout(title=f"{mn} {yr} vs {yr-1} (Bar)",
                ...
                with lc:
                    fl = go.Figure()
                    fl.add_trace(go.Scatter(x=days, y=avgs, name=f"Forecast {yr}",
                                            line=dict(color=color, width=2.5),
                                            mode="lines+markers", marker=dict(size=5),
                                            fill="tozeroy", fillcolor=fill))
                    if has_a and act_a:
                        fl.add_trace(go.Scatter(x=days, y=act_a, name=f"Actual {yr}",
                                                line=dict(color='#16a34a', width=2.5),
                                                mode="lines+markers",
                                                marker=dict(size=4), linedash="dot"))
                    if prev_avgs:
                        fl.add_trace(go.Scatter(x=days, y=prev_avgs,
                                                name=f"{yr - 1} Actual",
                                                line=dict(color=YEAR_COLORS.get(yr - 1, '#94a3b8'),
                                                          width=1.8, dash="dash"),
                                                mode="lines+markers", marker=dict(size=4)))
                    fl.add_trace(go.Scatter(x=days, y=peaks, name="Forecast Peak",
                                            line=dict(color="#dc2626", width=1.5, dash="dot"),
                                            mode="lines"))
                    fl.update_layout(title=f"{mn} {yr} vs {yr-1} (Line)",
                                     xaxis_title="Day", yaxis_title="Load (MW)",
                                     xaxis=dict(tickmode='linear', tick0=1, dtick=1),
                                     height=360, **BL)
                ...
                # Day picker
                st.markdown(f"**🔍 View a specific day in {mn} {yr}**")
                sel   = st.slider(f"Select day — {mn}", 1, ndays, 1, key=f"sl_{yr}_{mo}")
                drow  = df_mo[df_mo['day'] == sel]
                if len(drow) > 0:
                    dr  = drow.iloc[0]
                    dp  = g24(dr, 'pred'); dp = [v if v else 0 for v in dp]
                    da  = g24(dr, 'actual') if has_a else [None] * 24
                    ...
                    with db2:
                        fdb = go.Figure()
                        fdb.add_trace(go.Bar(x=hlbl, y=dp, name="Forecast",
                                             marker_color=color, opacity=0.85))
                        if has_a and any(v for v in da if v):
                            fdb.add_trace(go.Bar(x=hlbl, y=da, name="Actual",
                                                 marker_color='#16a34a', opacity=0.55))
                        fdb.update_layout(title=f"{mn} {sel} — Hourly (Bar)",
                                          xaxis_title="Hour", yaxis_title="Load (MW)",
                                          barmode='group', height=280, **BL)
                        st.plotly_chart(fdb, use_container_width=True)
                    with dl2:
                        fdl = go.Figure()
                        fdl.add_trace(go.Scatter(x=hlbl, y=dp, name="Forecast",
                                                  line=dict(color=color, width=2.5),
                                                  mode="lines+markers",
                                                  marker=dict(size=7, symbol="diamond"),
                                                  fill="tozeroy", fillcolor=fill))
                        if has_a and any(v for v in da if v):
                            fdl.add_trace(go.Scatter(x=hlbl, y=da, name="Actual",
                                                      line=dict(color='#16a34a', width=2.5),
                                                      mode="lines+markers",
                                                      marker=dict(size=5)))
                        if vdp:
                            ...
                            fdl.add_annotation(x=hlbl[ph], y=max(vdp),
                                               ...
                                               bgcolor="white",
                                               bordercolor=color, borderwidth=1, ay=-35)
    ...
    with tab4:
        ...
        st.caption("Historical data (2020–2025) embedded. 2026 forecast appears after running Colab.")
        sel_mn = st.selectbox("Select Month", ["April", "May", "June"], key="s5")
        sel_mo = {"April": 4, "May": 5, "June": 6}[sel_mn]
        color  = MONTH_COLORS.get(sel_mo, '#2563eb')
        ndays  = calendar.monthrange(2026, sel_mo)[1]
        df_mo26 = load_mo(sel_mn, 2026)

        # Daily line all years
        fig1 = go.Figure()
        ...
        if df_mo26 is not None and len(df_mo26) > 0:
            ...
            fig1.add_trace(go.Scatter(x=df_s['day'].tolist(),
            ...
            if 'actual_avg' in df_mo26.columns and df_mo26['actual_avg'].notna().any():
                fig1.add_trace(go.Scatter(x=df_s['day'].tolist(),
                                          y=pd.to_numeric(df_s['actual_avg'], errors='coerce').tolist(),
                                          name='2026 Actual',
                                          line=dict(color='#16a34a', width=2.5, dash='dash'),
                                          mode='lines+markers', marker=dict(size=5)))
        fig1.update_layout(title=f"{sel_mn} — Daily Avg Load 2020–2026",
        ...
        if df_mo26 is not None and len(df_mo26) > 0:
            yls.append("2026\n(Forecast)")
            yas.append(float(pd.to_numeric(df_mo26['predicted_avg'], errors='coerce').mean()))
            yps.append(float(pd.to_numeric(df_mo26['predicted_peak'], errors='coerce').max()))
            ycs.append(YEAR_COLORS[2026])
        else:
            yls.append("2026\n(Pending)"); yas.append(None)
            yps.append(None); ycs.append('#e5e7eb')
        fig2 = go.Figure()
        fig2.add_trace(go.Bar(x=yls, y=yas, name="Monthly Avg",
                              marker_color=ycs, opacity=0.88,
                              text=[f"{v:,.0f}" if v else "Run Colab" for v in yas],
                              textposition='outside'))
        fig2.add_trace(go.Scatter(x=yls, y=yps, name="Monthly Peak",
                                  mode='lines+markers',
                                  line=dict(color='#dc2626', width=2, dash='dot'),
                                  marker=dict(size=9, symbol='triangle-up', color='#dc2626')))
        fig2.update_layout(title=f"{sel_mn} — Monthly Avg & Peak by Year",
    ...
    # ── TAB 5: ACCURACY ──────────────────────────────────────
    with tab5:
        st.subheader("🎯 Forecast Accuracy — Months with Actual Data")
        if df_roll is None or len(df_roll) == 0:
            st.info("Accuracy data appears after running Colab with actual month files.")
        else:
            df_acc = df_roll[df_roll['mape'].notna()].copy() if 'mape' in df_roll.columns else pd.DataFrame()
            if len(df_acc) == 0:
                st.info(
                    "No accuracy metrics yet.\n\n"
                    "To compute MAPE/RMSE:\n"
                    "1. Upload `actual_MONTH_YEAR.csv` in the Colab notebook config\n"
                    "2. Re-run the notebook\n"
                    "3. Push results to GitHub")
            else:
                overall_mape = df_acc['mape'].mean()
                overall_rmse = pd.to_numeric(df_acc.get('rmse', []), errors='coerce').mean()
                a1, a2, a3 = st.columns(3)
                a1.metric("Overall Avg MAPE", f"{overall_mape:.2f}%")
                a2.metric("Overall Avg RMSE", f"{overall_rmse:.0f} MW"
                           if not np.isnan(overall_rmse) else "—")
                a3.metric("Days with Actual", len(df_acc))

                # MAPE trend
                c1, c2 = st.columns(2)
                with c1:
                    fm = px.line(df_acc, x="date", y="mape",
                                 title="MAPE % Over Time",
                                 markers=True, color_discrete_sequence=["#ea580c"])
                    fm.add_hline(y=overall_mape, line_dash="dash", line_color="red",
                                 annotation_text=f"Avg:{overall_mape:.2f}%")
                    fm.update_layout(height=320,
                                     plot_bgcolor="rgba(0,0,0,0)",
                                     paper_bgcolor="rgba(0,0,0,0)")
                    st.plotly_chart(fm, use_container_width=True)
                with c2:
                    if 'rmse' in df_acc.columns:
                        fr = px.line(df_acc, x="date", y="rmse",
                                     title="RMSE (MW) Over Time",
                                     markers=True, color_discrete_sequence=["#7c3aed"])
                        fr.add_hline(y=overall_rmse, line_dash="dash", line_color="red",
                                     annotation_text=f"Avg:{overall_rmse:.0f} MW")
                ...
                # Per-month breakdown
                if 'month_name' in df_acc.columns:
                    st.subheader("Per-Month Accuracy")
                    mo_acc = (df_acc.groupby(['year', 'month', 'month_name'])
                              .agg(days=('mape', 'count'),
                                   avg_mape=('mape', 'mean'),
                                   best_mape=('mape', 'min'),
                                   worst_mape=('mape', 'max'))
                              .reset_index())
                    mo_acc['avg_mape'] = mo_acc['avg_mape'].round(2)
                    mo_acc['best_mape'] = mo_acc['best_mape'].round(2)
                    mo_acc['worst_mape'] = mo_acc['worst_mape'].round(2)
                    st.dataframe(mo_acc, use_container_width=True, hide_index=True)

    # ── TAB 6: ALL RESULTS ───────────────────────────────────
    with tab6:
        if df_roll is None or len(df_roll) == 0:
            st.info("Results appear after running Colab.")
        else:
            st.subheader(f"All Results — {len(df_roll)} rows")
            base_cols = ['date', 'month_name', 'year', 'day',
                         'has_actual', 'mape', 'rmse',
                         'predicted_avg', 'predicted_peak',
                         'actual_avg', 'actual_peak']
            avail = [c for c in base_cols if c in df_roll.columns]
            ...
            st.download_button(
                ...
                "TN_rolling_results.csv", "text/csv",
...
def main():
    ...
    if not st.session_state['logged_in']:
        ...
        return
    status = load_status()
    show_sidebar(st.session_state['username'], st.session_state['role'], status)
